...
{
	return Complex(n * other.real, n * other.image);	// 멤버변수가 private이기 때문에 접근할 수 없음
}

// 출력 연산자 오버로딩
inline std::ostream& operator<<(std::ostream& out, const Complex &other)
{ 
	out << other.real << ", " << other.image << std::endl;
	return out;
}
...
int main()
{
	...
	// cout 객체를 통한 객체 출력
	std::cout << c << std::endl;					//위에서 선언 해줌 // 원래는 출력 연산자(<<)는 기본적으로 객체를 출력할 수 없다.
